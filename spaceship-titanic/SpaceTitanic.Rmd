...
Then we load in the train data set by separating the sets data points apart, creating fields that we will use for calculations and remove the name from the field
-Split cabin into three parts: Deck, Number and Side
-Create spent field that holds RoomService + FoodCourt + ShoppingMall + Spa + VRDeck
-Change Transported, CryoSleep & VIP T/F to 1 and 0 respectfully 
...
train <- 
    ...
    mutate(TransportedFlag = ifelse(Transported, 1, 0), 
           CryoSleepFlag = ifelse(CryoSleep, 1, 0), 
           VIPFlag = ifelse(VIP, 1, 0)) %>%
    select(PassengerId:CryoSleep, CryoSleepFlag, Deck:VIP, 
          VIPFlag, RoomService:VRDeck, Spent,Transported, TransportedFlag)
head(train)
spec(train)
train
```
...
We then change the missing values to be replaced with the most common value that the column holds
```{r}
getmode <- function(v) {
 uniqv <- unique(v)
...
train[["CryoSleepFlag"]][is.na(train[["CryoSleepFlag"]])] <- as.numeric(getmode(train$CryoSleepFlag))
...
train[["VIPFlag"]][is.na(train[["VIPFlag"]])] <- as.numeric(getmode(train$VIPFlag))
...
train[["TransportedFlag"]][is.na(train[["TransportedFlag"]])] <- as.numeric(getmode(train$TransportedFlag))
...
Look at the Transported against CryoSleep
```{r}
...
train %>%
    ggplot(aes(x=Transported, fill=CryoSleep)) +
        geom_bar()

```

Looking at the Transported individuals against the Amount they spent across the board
```{r}
options(repr.plot.width = 48, repr.plot.height = 15) 

train %>%
    ggplot(aes(x=Spent, color=Transported)) +
        geom_boxplot()
```

Look at the data from the Side and the Deck
```{r}
table(train$Deck, train$Side)
```

Looking at the Transported individuals against the Deck they were on
```{r}
table(train$Deck, train$Transported)
```

Looking at the Transported individuals against the Side they were on
```{r}
table(train$Side, train$Transported)
```

Home Planet and Destination
Looking at the Transported individuals against their Home Planet 
```{r}
table(train$HomePlanet, train$Transported)
```

Looking at the Transported individuals against their Destination 
```{r}
table(train$Destination, train$Transported)
```
Looking at the Home Planet of individuals against their Destination
```{r}
table(train$HomePlanet, train$Destination)
```

Age Data

Looking at the Transported individuals against their Age 
```{r}
...
#relationships <- 
#    train %>% 
#      select('CryoSleepFlag','VIPFlag','Age','RoomService', #'FoodCourt','ShoppingMall','Spa','VRDeck','Spent','TransportedFlag') %>%
#      cor()
#ggcorrplot(train, type="upper", lab=TRUE) + theme(text = element_text(size = 20))
#
```

```{r}
#round(relationships, 2)
```
...
To summarize this section we could say that there is some strong correlation between certain data points and if they were transported or not. We saw that majority of people in Cyposleep were transported, that the amount they spent played a role but not too large, that the side one was one seemed to have an affect, that certain side of the deck one was on could have played a role, the planet one was traveling to played a role, the age they are played a role and the planet they were from played a role.

From this information we conclude that there will a higher change of those who were transported were under the age of 10 and over the age of 30, that spent less than other passages, lived on Europa and Mars, were traveling to 55 Cancri e & PSO J318.5-22, who were on side P and who were on Deck C and D
...
Help with understanding on how to start and what to look for in the data set
